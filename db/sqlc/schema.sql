...
CREATE TABLE workers (
  ...
  strength INTEGER NOT NULL CHECK (
    strength >= 1
    AND strength <= 10
  ),
  intelligence INTEGER NOT NULL CHECK (
    intelligence >= 1
    AND intelligence <= 10
  ),
  speed INTEGER NOT NULL CHECK (
    speed >= 1
    AND speed <= 10
  ),
  faith INTEGER NOT NULL CHECK (
    faith >= 1
    AND faith <= 10
  ),
  luck INTEGER NOT NULL CHECK (
    luck >= 1
    AND luck <= 10
  ),
  loyalty INTEGER NOT NULL CHECK (
    loyalty >= 1
    AND loyalty <= 10
  ),
  charisma INTEGER NOT NULL CHECK (
    charisma >= 1
    AND charisma <= 10
  ),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_eggs_unhached ON eggs (worker_id)
WHERE
  worker_id IS NULL;
