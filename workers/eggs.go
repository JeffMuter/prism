...
func GetEggsAvailableForUser(userId int) ([]Egg, error) {
	var eggs []Egg
	db := db.GetDB()
	query := "SELECT e.id, COALESCE(ul.name, '') FROM eggs e JOIN users_locations ul ON ul.id = e.users_locations_id JOIN users u ON u.id = ul.user_id JOIN locations l ON ul.location_id = l.id WHERE u.id = ? AND e.worker_id IS NULL"
